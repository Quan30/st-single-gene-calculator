...
def _precompute_static_structures(model, mdata_real, cfg, reference_stats):
    ...
    gene_names_origin = list(mdata_real["gene_name"])
...
def power_vs_cells(cells_min, cells_max, n_bins, cfg: SimulationConfig, tcfg: TestConfig,
    ...
    for n_cpe in xs:
        ...
        if abort():
            ...
            return _detail_to_power_summary(list_dict, grouping_columns, cfg, tcfg)  # <-- this util already exists in your file
    ...
    power_summary = _detail_to_power_summary(list_dict, grouping_columns, cfg, tcfg)
